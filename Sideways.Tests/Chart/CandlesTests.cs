    ...
    public static class CandlesTests
    {
        ...
        public static void Adr()
        {
            var candles = new Candles(
                ...
                default);
            Assert.AreEqual("10.8%", candles.Adr(new DateTimeOffset(2021, 6, 1, 0, 0, 0, 0, TimeSpan.Zero))?.ToString("#.#", CultureInfo.InvariantCulture));
        }
        ...
        public static void Atr()
        {
            var candles = new Candles(
                SortedCandles.Create(
                    ...
                    /*20*/ new Candle(time: new DateTimeOffset(2021, 6, 1, 0, 0, 0, 0, TimeSpan.Zero), open: 27.95f, high: 29.85f, low: 27.5f, close: 28.54f, volume: 2857094)),
                default);
            Assert.AreEqual("2.4", candles.Atr(new DateTimeOffset(2021, 6, 1, 0, 0, 0, 0, TimeSpan.Zero))?.ToString("#.#", CultureInfo.InvariantCulture));
        }
