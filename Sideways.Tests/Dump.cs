namespace Sideways.Tests
{
    ...
    public static class Dump
    {
        ...
        [Test]
        public static void DaysCreateMultiline()
        {
        ...
        [Test]
        public static void DaysCreateSingleLine()
        {
            foreach (var day in Database.ReadDays("EH", new DateTimeOffset(2021, 05, 01, 00, 00, 00, TimeSpan.Zero), new DateTimeOffset(2021, 06, 02, 00, 00, 00, TimeSpan.Zero)))
            {
                Console.WriteLine($"new Candle(time: new DateTimeOffset({day.Time.Year}, {day.Time.Month}, {day.Time.Day}, {day.Time.Hour}, {day.Time.Minute}, {day.Time.Second}, 0, TimeSpan.Zero), open: {day.Open.ToString(CultureInfo.InvariantCulture)}f, high: {day.High.ToString(CultureInfo.InvariantCulture)}f, low: {day.Low.ToString(CultureInfo.InvariantCulture)}f, close: {day.Close.ToString(CultureInfo.InvariantCulture)}f, volume: {day.Volume}),");
            }
        }
    }
}
